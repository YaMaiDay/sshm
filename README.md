...
sshm 是本地运行的 SSH 管理工具，默认不在后台连网检查更新，也不会向项目服务器上报服务器信息。
...
sshm is a local terminal SSH manager. It does not include telemetry or runtime update checks.

| Behavior | Status |
| --- | --- |
| Automatic update checks | No |
| Telemetry / analytics | No |
| Crash reporting | No |
| Background requests to GitHub | No |
| Project-owned server contacted at runtime | No |
| Uploading server list, IPs, usernames, paths, command history, or transfer history | No |

Network access only happens when the user explicitly performs an action:

- Running `install.sh` / `install.ps1` downloads release assets from GitHub Releases.
- Connecting to a user-configured SSH server for monitoring, login, command execution, upload, or download.
- Confirming remote `rsync` installation, which runs the remote system package manager on the user's server.
