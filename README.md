...
`sshm` 是给经常连服务器的人用的终端工作台。你不用在 SSH 配置、密码文件、监控命令、`scp` 命令之间来回切，打开一个面板就能完成常用运维动作。
...
- **看状态**：CPU、内存、磁盘、负载、容器、异常服务、健康端口一屏扫完。
- **进机器**：直接调用系统 `ssh`，进入后就是原生终端，`vim`、`tmux`、Tab 补全都正常。
- **管服务器**：分类、收藏、备注、到期时间、复制服务器，适合服务器越来越多的场景。
- **查风险**：详情页汇总失败登录、SSH 密码登录、Root 登录、端口和容器风险。
- **跑命令**：保存全局/单服务器命令模板，也可以批量在多台服务器串行执行。
- **传文件**：不用手写 `scp`，左右选择本地和远程路径后上传/下载。

## ✨ 为什么用 sshm

如果你习惯待在终端里，`sshm` 可以把“找服务器、看状态、登录、传文件、执行命令”这些动作压到一个入口里。它不会内置远程 shell，登录时直接调用系统 `ssh`，并把真实终端交给 SSH 进程，所以远程 Tab 补全、vim、tmux、Ctrl+C、删除键都保持原生体验。

| 你以前可能这样 | 现在可以这样 |
| --- | --- |
| 到处找服务器 IP、端口、用户名 | 在 TUI 里统一添加和分类 |
| 服务器用途全靠记忆 | 给服务器加备注，卡片、详情和搜索里都能看到 |
| 新机器配置和旧机器很像 | 复制服务器后只改名称、IP 或密钥 |
| 登录后手动执行 `top`、`df`、`free` | 主面板直接看 CPU、内存、磁盘、负载 |
| 日常部署命令反复敲 | 保存命令模板，单台或批量执行 |
| 想看是否有人扫 SSH | 详情页汇总成功/失败登录和安全建议 |
...
| 场景 | 能力 |
| --- | --- |
| 服务器总览 | 全中文 TUI、卡片视图、分类视图、搜索、筛选、排序、收藏 |
| 资源监控 | CPU、内存、Swap、磁盘、inode、负载、运行时间、系统信息 |
| 服务状态 | 健康端口、监听端口、异常 systemd 服务、Docker 运行/停止/故障容器 |
| 服务器管理 | 添加、复制、编辑、删除、分类、备注、到期时间、跳板机 |
| 登录安全 | 成功/失败登录摘要、密码登录检查、Root 登录检查、风险建议 |
| 日常操作 | 系统 `ssh` 登录、文件/目录上传下载、命令模板、批量命令、命令历史 |
| 数据迁移 | 从 OpenSSH 配置和旧密码文件迁移到 `servers.toml` |
| 平台 | macOS / Linux 推荐，Windows Terminal + OpenSSH 实验性支持 |
