...
sshm 是本地终端 SSH 管理工具，不包含遥测，也不会在运行时自动检查更新。
...
| 后台访问 GitHub | 否 |
| 运行时连接项目方服务器 | 否 |
| 上传服务器列表、IP、用户名、路径、命令历史或传输历史 | 否 |

只有用户明确执行操作时才会产生网络访问：

- 运行 `install.sh` / `install.ps1` 安装脚本时，会从 GitHub Releases 下载程序。
- 连接用户自己配置的 SSH 服务器，用于监控、登录、执行命令、上传或下载。
- 用户确认远程安装 `rsync` 时，会在用户自己的服务器上调用系统包管理器。
